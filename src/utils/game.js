...
export async function endGame(guesses, score, uuid) {
    const text = await query2("SELECT * FROM Records WHERE username = $1", [uuid]);
    var match = t.rows[0];
    if (!match) {
    ...
    // commit to db
    // return await query(query_text, [new_high_score, new_avg_score, new_games_played, new_within_10_feet, new_over_1000_miles, new_five_games_played, new_ten_games_played, uuid]);
